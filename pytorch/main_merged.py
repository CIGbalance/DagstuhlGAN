...
#SORT THEM IN ARRAY BY TYPE. E.G. FREE, ..... Wall at the end
# "tiles" : {
#     0    "X" : ["solid","ground"],
#     1    "S" : ["solid","breakable"],
#     2    "-" : ["passable","empty"],
...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    opt.nz = 32#16*2 #was 16
    opt.batchSize = 32 #was 32
...
#opt.batchSize = 30
    ...
    X_onehot = X#np.eye(z_dims, dtype='uint8')[X]
    X_onehot = X_onehot.reshape(X.shape[0], 1, X.shape[1], X.shape[2])
    print (X_onehot.shape)    #(173, 14, 28, 16)

    #print(data.shape) #(173, 14, 28)
    X_train = np.ones ( (X.shape[0], z_dims, map_size, map_size) )*2

    #Pad part of level so its a square
    X_train[:X.shape[0], :, :X.shape[1], :X.shape[2]] = X_onehot

    empty = ( X_train == 2. )
    impassible = ( X_train != 0.0)

    #X_train = 1.
    X_train[ X_train == 2. ] = -1.   #Replace free space with -1
    X_train[ X_train != -1. ] = 1.

    #mean_X = np.mean(X_train)

    #X_train = (X_train - mean_X) / np.std(X_train)
    #print (X_train)
    #exit()
    #X_train[empy] = 0.0 #GROUND
    #X_train[impassible] = 0.0
    ...
    #!X_train = X_train / 12.0
    #!X_train = (X_train - 0.164819288411) / 0.0594355658913
    
    #print (np.min(X_train)) -2.77307510982
    #exit()
...
if opt.mlp_D:
    netD = mlp.MLP_D(opt.imageSize, nz, z_dims, ndf, ngpu)
else:
...
for epoch in range(opt.niter):
    ...
    while i < num_batches:#len(dataloader):
        ...
        while j < Diters and i < num_batches:#len(dataloader):
            ...
            if (False):
                ...
                vutils.save_image(real_cpu, '{0}/real_samples_{1}.png'.format(opt.experiment, gen_iterations))
                exit()
        ...
        if gen_iterations % 10 == 0:   #was 500
            ...
            if (True):
                ...
                #fake.data = np.round(fake.data*10.0, decimals=0)/10

                fake.data[fake.data > 0.] = 1.
                fake.data[fake.data < 0.] = -1.


                #fake.data[fake.data > mean_X] = 1.
                #fake.data[fake.data < mean_X] = -1.

                
                fake.data = fake.data.mul(0.5).add(0.5)
                #fake.data = np.round(fake.data, decimals=1)

                #print ( fake.data*14.0 )
                #print('SUM ',np.sum( im, axis = 1) )
                #fake.data = (fake.data.mul(0.5).add(0.5))#*10)/6.0
                #fake.data = fake.data.mul(0.5).add(0.5)
               
                #print( )#/10 )
                #fake.data = np.round(fake.data*14.0, decimals=0)/14
                vutils.save_image(fake.data, '{0}/fake_samples_{1}.png'.format(opt.experiment, gen_iterations))
                #im = combine_images( tiles2image( im)  )
    ...
    torch.save(netG, 'mario_gan_2.pth')
