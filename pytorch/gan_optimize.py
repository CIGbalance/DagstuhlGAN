...
def combine_images(generated_images):
    ...
    print("SHAPE ", shape)
    image = numpy.zeros((height * shape[1], width * shape[2], shape[3]), dtype=generated_images.dtype)
    for index, img in enumerate(generated_images):
        i = int(index / width)
        j = index % width
        image[i * shape[0]:(i + 1) * shape[1], j * shape[1]:(j + 1) * shape[2]] = img
    return image
...
def ground_blocks_fraction(data, frac):
    ground_count = sum(data[0, 0, 13, :] > 0)
    #print(ground_count)
